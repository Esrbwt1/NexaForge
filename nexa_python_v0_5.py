...
def extract_features(node):
    ...
    if isinstance(node, ast.For):
        if isinstance(node.iter, ast.Call) and node.iter.func.id == 'range':
            ...
            features[7] = 1 if features[0] > 10000000 else 0
        features[3] = 1
...
def threaded_sum_even(limit):
    chunk_size = limit // 2
    with Pool(2) as p:
        results = p.starmap(thread_chunk, [(0, chunk_size), (chunk_size, limit)])
    return sum(results)
...
if __name__ == '__main__':
    ...
    # Test NexaScript
    nexa_script = "NEXASCRIPT: PARALLEL(1000000)"
