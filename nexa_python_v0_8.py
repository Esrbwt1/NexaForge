...
def extract_features(node):
    ...
    if isinstance(node, ast.For):
        if isinstance(node.iter, ast.Call) and node.iter.func.id == 'range':
            ...
            features[7] = 1 if features[0] >= 10000000000 else 0
        features[3] = 1
...
def fast_code():
    total = np.sum(np.arange(0, 1000000000, 2, dtype=np.int64))
    return total
...
if __name__ == '__main__':
    ...
    # Test NexaScript
    nexa_script = "NEXASCRIPT: PARALLEL(1000000000)"
    runtime_nexa = NexaRuntime("NexaForge2025")
    print("Running NexaScript in NexaRuntime...")
    nexa_result, nexa_telemetry = runtime_nexa.run_nexa(parse_nexascript(nexa_script))
    ...
    # Output
    print(f"Slow: {slow_result}, Time: {slow_time}")
    print(f"Fast (Python): {fast_result}, Time: {python_time}")
    print(f"Telemetry (Python): {{'runs': 1, 'time': {python_time}, 'optimizations': 1}}")
